...
let AuthService = class AuthService {
    ...
    token;
    otpService;
    constructor(userRepo, crypto, token, otpService) {
    ...
    async signIn(dto) {
        ...
        if (!user.isActive) {
            throw new common_1.UnauthorizedException('Akkaunt faol emas. Administratorga murojaat qiling');
        }
        ...
        return (0, succes_res_1.succesRes)({ token, user: safeUser });
    }
    async requestOtpLogin(phone) {
        return this.otpService.sendOtp(phone);
    }
    async verifyOtpLogin(phone, code) {
        ...
        if (!user.isActive) {
            throw new common_1.UnauthorizedException('Akkaunt faol emas');
        }
        const payload = {
            id: user.id,
            role: user.role,
