...
let AdminService = class AdminService {
    ...
    async create(dto) {
        ...
        const user = this.userRepo.create({ ...dto, password: hashed, role: role_enum_1.UserRole.ADMIN });
    ...
    async findAll() {
        const users = await this.userRepo.find({
            where: { role: role_enum_1.UserRole.ADMIN },
            relations: ['direction'],
    ...
    async findOne(id) {
        const user = await this.userRepo.findOne({ where: { id, role: role_enum_1.UserRole.ADMIN }, relations: ['direction'] });
        if (!user)
            throw new common_1.NotFoundException(`Admin ID ${id} topilmadi`);
        const { password: _, ...result } = user;
        return (0, succes_res_1.succesRes)(result);
    }
    async update(id, dto) {
        const user = await this.userRepo.findOne({ where: { id, role: role_enum_1.UserRole.ADMIN } });
        if (!user)
            throw new common_1.NotFoundException(`Admin ID ${id} topilmadi`);
    ...
    async remove(id) {
        ...
        return (0, succes_res_1.succesRes)({ message: 'Admin muvaffaqiyatli ochirildi' });
    }
};
